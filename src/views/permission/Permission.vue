      ...
      >
        <el-form-item prop="studentId">
          ...
          <el-input
            ...
            @clear="resetForm"
            @input="init"
            @keyup.enter.native="search(form.studentId)"
          >
            <el-button
              slot="append"
              icon="el-icon-search"
              @click="search(form.studentId)"
              :loading="btnLoading"
            ></el-button>
          </el-input>
        </el-form-item>
        <el-form-item label="权限配置">
          ...
          <template v-if="selectRoles.length && !isCommon">
            ...
            <el-button
              ...
              title="添加权限"
              @click="showSelect"
              class="new-tag tag el-icon-plus"
            ></el-button>
          </template>
        </el-form-item>
        <el-form-item align="center">
          <el-button
            ...
            round
            @click="change"
            :disabled="check || !selectedRole || !isChange"
          >设置权限</el-button>
        </el-form-item>
      </el-form>
    </el-card>
      ...
      this.$confirm(`是否分配权限给 ${this.form.studentId} ？`, "提示", {
        ...
        .then(() => {
          ...
          changeRole(formData)
            ...
            .catch(err => this.$message.error("权限设置失败"));
        })
