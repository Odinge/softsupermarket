...
import { ADDROUTER, SET_TOKEN, SET_ROLE, CLEAR_STATE, SET_TEAM_DIRECTION, SET_MESSAGE, SET_MSG_NUM, SET_USER_INFO, SET_USERNAME, SET_TEMPROLE } from './types';
import { toLogin, login, getUserInfor, getStudentIdByRequest, getUserId, getRole } from "../api/login.js";
import { getMessageById, getMessageByState, getMessageByUserGet, changeMessageState } from './../api/login';
...
export default {
    ...
    getRequest({ commit }) {
        return new Promise((resolve, reject) => {
            // 根据请求获取学号
            getStudentIdByRequest().then(res => {
                if (res.status == 0) {
                    // 获取用户id
                    let studentId = res.data;
                    getUserId(studentId).then(res => {
                        if (res.status == 0) {
                            // 获取权限
                            let userId = res.data;
                            ...
                            }).catch(err => reject({ err, msg: "权限获取失败", code: 3 }));
                        } else throw res.msg;
                    }).catch(err => reject({ err, msg: "没有此用户", code: 2 }));
                } else throw res.msg;
            }).catch(err => { reject({ err, msg: "无效登录", code: 1 }) });
        });
    },
    // 获取角色改变显示
    async getUserInfor({ commit }) {
        ...
        let roles = (await getRole(userId)).data;
    ...
    async initMsgNum({ commit }) {
        ...
        // 消息数量
        // data = (await getMessageByUserGet("asd96eb3e28437216e3bfd92e9c72fd1")).data || [];
